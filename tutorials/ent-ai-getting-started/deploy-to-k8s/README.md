...
8. Render the client-secret Secret and re-render the chart's OIDC ConfigMap so it carries the client ID, the secret reference, and the port-forward browser endpoint:
    ...
    ```bash
    kubectl -n ent-ai create secret generic oidc-client \
      --from-literal=clientSecret=$OIDC_APP_SECRET \
      --dry-run=client -o yaml | kubectl apply -f -

    helm upgrade ent-ai ./charts/portfolios/ent-ai --namespace ent-ai --reuse-values \
...
9. Hydrate OIDC env vars from the `ent-ai-oidc` ConfigMap (consumed by the next section's `rad deploy`):
    ...
    ')"
    export OIDC_CLIENT_SECRET=$(kubectl -n ent-ai get secret "$OIDC_CLIENT_SECRET_NAME" \
