...
- **Optional two-AKS workshop fallback:** If the event will use one AKS cluster to represent the local/edge environment and a second AKS cluster for the Azure environment, validate quota for both clusters before the hack and agree on distinct context names (`<local-aks-context>` and `<azure-aks-context>`). This is an optional portability teaching shortcut, not an Azure Local substitute.
