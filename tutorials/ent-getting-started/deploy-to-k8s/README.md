...
2.  Bootstrap the infrastructure. This sets up a local K3s cluster, installs Radius (control plane + resource types + group + environment), installs the `ent` Helm chart (Keycloak + Istio + observability + governance), automates Keycloak client creation, and starts a port-forward on the Keycloak service.

    ```bash
    ada bootstrap --portfolio ent --platform k3s --release ent --namespace ent --with-radius --keep-port-forward
    ```
...
3. In another terminal window, execute the above `export` commands to set the environment variables.
    ...
    export OIDC_ISSUER=http://ent-keycloak.ent.svc.cluster.local:8080/realms/master
    export OIDC_AUTH_ENDPOINT=http://ent-keycloak.ent.svc.cluster.local:8080/realms/master/protocol/openid-connect/auth
    export OIDC_TOKEN_ENDPOINT=http://ent-keycloak.ent.svc.cluster.local:8080/realms/master/protocol/openid-connect/token
    export OIDC_USERINFO_ENDPOINT=http://ent-keycloak.ent.svc.cluster.local:8080/realms/master/protocol/openid-connect/userinfo
...
> **Optional:** After the app is running, head to [step 5 of OPTION 2](#5-try-an-opa-authorization-policy) to attach an OPA `AuthorizationPolicy` to the frontend and watch Istio + OPA reject requests carrying an `x-deny: true` header.
