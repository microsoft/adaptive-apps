...
2.  Bootstrap the rest of the stack. This installs Radius (control plane + resource types + group + environment), installs the `core-ai` Helm chart (Keycloak + Istio + observability + Kaito-aware overlays), automates Keycloak client creation, and starts a port-forward on the Keycloak service. The GPU-enabled K3s cluster and Kaito from the prerequisites above are reused as-is.

    ```bash
    ada bootstrap --portfolio core-ai --platform k3s --release core-ai --namespace core-ai --with-radius --keep-port-forward
    ```
...
3. In another terminal window, execute the above `export` commands to set the environment variables.
    ...
    export OIDC_ISSUER=http://core-ai-keycloak.core-ai.svc.cluster.local:8080/realms/master
    export OIDC_AUTH_ENDPOINT=http://core-ai-keycloak.core-ai.svc.cluster.local:8080/realms/master/protocol/openid-connect/auth
    export OIDC_TOKEN_ENDPOINT=http://core-ai-keycloak.core-ai.svc.cluster.local:8080/realms/master/protocol/openid-connect/token
    export OIDC_USERINFO_ENDPOINT=http://core-ai-keycloak.core-ai.svc.cluster.local:8080/realms/master/protocol/openid-connect/userinfo
...
### 4. Install Adaptive App Capability Portfolio (Core-AI)
...
8. Build the OIDC env vars for the next section's `rad deploy`. The chart's `core-ai-oidc` ConfigMap already exposes the in-cluster endpoint URLs; the client ID/secret come straight from Keycloak, and the browser endpoint is the port-forward URL:
