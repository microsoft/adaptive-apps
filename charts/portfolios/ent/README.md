...
enforcement to the service mesh. The capability added by this chart is a
centralized [Open Policy Agent (OPA)](https://www.openpolicyagent.org/)
deployment running the [opa-envoy-plugin](https://github.com/open-policy-agent/opa-envoy-plugin),
...
|-------|---------|-------------|
| `opa.enabled` | `true` | Install OPA in the release namespace. |
...
| `istioExtAuthz.enabled` | `true` | Register OPA with Istio as an `extensionProvider`. |
